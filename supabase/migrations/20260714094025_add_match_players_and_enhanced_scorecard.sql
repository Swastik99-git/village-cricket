/*
# Match Players, Enhanced Scorecard, Google OAuth Support

## Overview
1. Creates `match_players` table to store selected players per match.
2. Adds enhanced scorecard columns to `match_performances`: fours, sixes, catches, run_outs.
3. Adds match-level columns: winning_margin, man_of_match_player_id.
4. Adds `provider` column to profiles for tracking auth provider (email vs google).
5. Drops and recreates `player_career_stats` view to include new fields.
6. Updates the signup trigger to handle Google OAuth.

## New Tables
### match_players
- id, match_id (FK), player_id (FK), created_at
- Unique constraint on (match_id, player_id)

## Modified Tables
### match_performances: + fours, sixes, catches, run_outs (all int default 0)
### matches: + winning_margin (text), man_of_match_player_id (uuid FK nullable)
### profiles: + provider (text default 'email')

## Security
- match_players: all authenticated read; admin-only insert/delete.

## Notes
1. View is dropped and recreated (can't ALTER columns in-place).
2. Trigger handles both email and Google signups.
*/
