...
import { ChevronLeft, MapPin, Calendar, Trophy, Award } from 'lucide-react';
import Layout from '../components/Layout';
import PlayerAvatar from '../components/PlayerAvatar';
import Spinner, { EmptyState, ErrorState } from '../components/Feedback';
import { useMatch, useMatchPerformances } from '../hooks/useQueries';
...
export default function MatchDetails() {
  ...
  const bowlingPerformances = (performances ?? [])
    ...
    .sort((a, b) => (a.bowling_order || 999) - (b.bowling_order || 999));

  return (
    <Layout>
      <div className="page-container">
        <Link to="/matches" className="inline-flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400 mb-4 hover:text-cricket-600 dark:hover:text-cricket-400 transition-colors">
            ...
            {perfLoading ? (
              ...
              <>
                ...
                {battingPerformances.length > 0 && (
                  <div className="mb-5">
                    ...
                    <div className="card overflow-hidden">
                      <div className="grid grid-cols-12 gap-2 px-4 py-2.5 bg-gray-50 dark:bg-gray-800 text-xs font-semibold text-gray-400 uppercase">
                        <div className="col-span-5">Player</div>
                        <div className="col-span-2 text-center">R</div>
                        <div className="col-span-2 text-center">B</div>
                        <div className="col-span-3 text-right">Dismissal</div>
                      </div>
                      {battingPerformances.map((p) => (
                        <div key={p.id} className="grid grid-cols-12 gap-2 px-4 py-3 border-t border-gray-100 dark:border-gray-800 items-center">
                          ...
                          <div className="col-span-2 text-center text-sm font-bold text-gray-900 dark:text-white">
                            {p.runs}{!p.is_out && p.balls_faced > 0 ? '*' : ''}
                          </div>
                          <div className="col-span-2 text-center text-sm text-gray-500 dark:text-gray-400">
                            {p.balls_faced}
                          </div>
                          <div className="col-span-3 text-right text-xs text-gray-400 truncate">
                            {p.dismissal || 'not out'}
                          </div>
                        </div>
                      ))}
                ...
                {bowlingPerformances.length > 0 && (
                  <div className="mb-5">
                    <h2 className="section-title mb-3 flex items-center gap-2">
                      <Award className="w-5 h-5 text-cricket-500" /> Bowling
                    </h2>
                    <div className="card overflow-hidden">
                      <div className="grid grid-cols-12 gap-2 px-4 py-2.5 bg-gray-50 dark:bg-gray-800 text-xs font-semibold text-gray-400 uppercase">
                        <div className="col-span-5">Player</div>
                      ...
                      {bowlingPerformances.map((p) => (
                        <div key={p.id} className="grid grid-cols-12 gap-2 px-4 py-3 border-t border-gray-100 dark:border-gray-800 items-center">
                          ...
                          <div className="col-span-2 text-center text-sm font-bold text-cricket-600 dark:text-cricket-400">
                            {p.wickets}
                          </div>
